...
this.execute = function () {
	...
	this.render = function (data, full_data) {
		...
		for(var i = 0; i < data_length; i++) {
			html += '<li class="list-group-item list_class col-sm-12 col-without-padding" id="' + i + '"><div class="amendments col-sm-11 col-without-padding" id="'+i+'"><span id="tour_'+i+1+'">Amendment '+(i+1)+'</span></div><div class="badge col-sm-1 col-without-padding" id="' + i + '">+</div><br><span class="summary close" id="summary-' + i + '">' + data[i].summary + '<div class="pull-right"> <a href="https://www.google.com/search?q=Amendment+' + (i + 1) + '+of+Indian+Constitution" target="_blank">Read More</a> </div></span></li>'
		}
		...
		var layout_heatmap = function () {
			...
			if (domain <= 0.1) {
				return "#e9e6d4"; // light gold
			} else if (domain <= 0.4) {
				return "#d0be87"; // classic gold
			} else {
				return "#c3b16b"; // deep antique gold
			}
						}
		...
		$('.article-list #parts_div .box').on('click',function () {
			...
			$.noty.closeAll()
				...
				$("#selected_ammendments").html("<span style='' class='pull-right'>Related Amendments</span>");
		...
		$('.article-list #parts_div .box').dblclick(function () {
			$('#detail').trigger('click');
		});

		$('.article-list #parts_div .box, .article-list #schedules .box').dblclick(function () {
    $('#detail').trigger('click');
});
